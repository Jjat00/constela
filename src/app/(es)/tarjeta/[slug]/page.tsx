...
/** La tarjeta con y sin sesión habla el mismo idioma de datos. */
type PerfilTarjeta = {
  id: string;
  name: string;
...
export async function generateMetadata({
  ...
  return {
    title: nombre ? `Tarjeta de ${nombre} — Constela` : "Tarjeta — Constela",
    ...NO_INDEXAR,
  };
}
 ...
 *
 * Abierta sin sesión —es el enlace que se comparte fuera del evento— pero
 * nunca indexada, igual que `/u/[slug]`: existe para quien recibe el link,
 * no para el archivo de un buscador. Solo enseña canales que ya son públicos
 * donde viven; el WhatsApp sigue detrás del encuentro real.
 */
export default async function TarjetaPage({
  ...
  // Con sesión la lectura es directa (RLS de authenticated) e incluye el
  // WhatsApp; sin sesión, el lookup público security definer expone solo
  // los canales públicos — nunca el teléfono.
  let profile: PerfilTarjeta | null = null;
  if (user) {
    const { data } = await supabase
      .from("profiles")
      .select(
        "id, name, headline, role, tags, intents, avatar_url, website, instagram, linkedin, github, whatsapp_number",
      )
      .eq("qr_slug", slug)
      .maybeSingle();
    profile = data;
    ...
    const p = data?.[0];
    if (p) profile = { ...p, whatsapp_number: null };
  }
  if (!profile) notFound();
  ...
  const rutaPublica = `/u/${slug}`;
  const urlPublica = `${siteUrl()}${rutaPublica}`;
  // Lo que se comparte es ESTA página: la tarjeta entera, con sus canales.
  const urlTarjeta = `${siteUrl()}/tarjeta/${slug}`;
  ...
  return (
    <main className="grain relative flex flex-1 flex-col">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-2xl flex-1 flex-col items-center gap-7 px-5 py-8 sm:px-8 sm:py-12">
        ...
        <div className="flex w-full max-w-md flex-col gap-2.5 sm:flex-row sm:justify-center">
          ...
          {isMe ? (
            ...
            // Quien llega sin cuenta acaba de recibir una tarjeta: la puerta
            // natural es hacerse la suya, no el login de una app ajena.
            <Link
              href="/"
              className="btn-cosmic inline-flex h-11 items-center justify-center px-5 text-[13px] font-medium"
            >
              Crea tu tarjeta en Constela
            </Link>
          )}
        </div>
      </div>
